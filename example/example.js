...
function simulateProgress1(current) {
    if (current <= TOTAL1) {
        ...
        setTimeout(() => {
            bar2 = new AsciiBar({
                formatString: "#spinner ##red #count #percent ##default#bar |##blue Time to finish: #ttf",
                total: TOTAL2,
                enableSpinner: true,
                hideCursor: true,
            });
...
function simulateProgress2(current) {
    if (current > 12) {
        bar2.formatString = "#spinner ##yellow #count #percent ##default#bar |##blue Time to finish: #ttf";
    }
    if (current > 27) {
        bar2.formatString = "#spinner ##green #count #percent ##default#bar |##blue Time to finish: #ttf";
    }
